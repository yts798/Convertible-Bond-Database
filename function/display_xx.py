...
for code in DTBS['A'].keys():
    if DTBS['A'][code][dt]['ia'] == 1 and DTBS['A'][code][dt]['xx30'] != 0:
        re.append((code, DTBS['B'][code]['cn'], DTBS['A'][code][dt]['xx'],DTBS['A'][code][dt]['xx15'],DTBS['A'][code][dt]['xx30']))
...
f = open(tdt+'-下修.txt','w', encoding='utf-8')
f.write('今日('+td+')数据库下修信息简报：\n')        
f.write('共有'+str(len(re))+'支转债近日满足过下修条件。\n')
